/* =====================================================
   DearBUP — Admin Dashboard  |  dashboard.js
   Save this as: js/dashboard.js

   ROUTES USED (matches your Express backend exactly):
   GET    /api/organization                              → getOrganizations
   GET    /api/organization/:id                          → getOrganizationById
   GET    /api/organization/:id/pending                  → getPendingMembers
   PATCH  /api/organization/:id/members/:uid/approve     → approveMember
   DELETE /api/organization/:id/members/:uid             → rejectMember
   GET    /api/events                                    → getEvents (paginated)
   GET    /api/events/stats                              → getEventStats
   DELETE /api/events/:id                                → deleteEvent
   GET    /api/cboOfficer/org/:orgId                     → getOrgOfficers
   POST   /api/cboOfficer/assign                         → assignOfficer
   ===================================================== */
...
/* ─────────────────────────────────────────────────────
   ...
   Keys saved by your saveAuthData():
     dearbup_token  → JWT string
     dearbup_user   → { user_type, role, ... }
   ───────────────────────────────────────────────────── */
...
function guardAdmin() {
  const user = getUser();
  // Check both user_type (new saveAuthData fix) AND role (old sessions)
  const ok = user && (user.user_type === 'admin' || user.role === 'admin');
  if (!ok) {
...
/* ─────────────────────────────────────────────────────
   MODAL HELPERS  (your CSS uses .active, not .open)
   ───────────────────────────────────────────────────── */
...
async function loadOverview() {
  try {
    ...
    const totalApproved = orgs.reduce(
      (n, o) => n + (o.members||[]).filter(m => m.status==='approved').length, 0
    );
    const totalPending = orgs.reduce(
      (n, o) => n + (o.members||[]).filter(m => m.status==='pending').length, 0
    );
...
/* ─────────────────────────────────────────────────────
   ORG DRAWER — GET /api/organization/:id
   ───────────────────────────────────────────────────── */
...
/* ── Approve  PATCH /api/organization/:id/members/:uid/approve ── */
...
/* ── Reject / Remove  DELETE /api/organization/:id/members/:uid ── */
...
/* ═════════════════════════════════════════════════════
   OFFICERS — GET /api/cboOfficer/org/:orgId
...
async function loadOfficers() {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      targetOrgs.map(org =>
        api.get(`/api/cboOfficer/org/${org._id}`)
...
async function submitAssignOfficer() {
  ...
  try {
    await api.post('/api/cboOfficer/assign', { org_id, user_id, officer_role });
...
/* ─────────────────────────────────────────────────────
   ORG DROPDOWNS — shared across all selects
   ───────────────────────────────────────────────────── */
...
function initEventListeners() {
  ...
  // Refresh button
  document.querySelector('.btn-refresh')?.addEventListener('click', loadOverview);
}
...
function initDashboard() {
  if (!guardAdmin()) return;   // redirects non-admins
  populateSidebar();
  initEventListeners();
  navigateTo('overview');
}
